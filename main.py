...
@bot.message_handler(
    commands=["start", "help"]
)  # реагирование бота на команды start & help
def command_start_help(
    message: telebot.types.Message,
):  # относится ли message типу класса Message
    text = (
    ...
    bot.reply_to(
        message, text
    )  # reply_to выделяет последнее сообщение пользователя и отвечает на него с ссылкой
...
@bot.message_handler(
    content_types=["text"]
)  # реагирование бота на текстовые сообщения от пользователей
...
def convert(message: telebot.types.Message):
    try:  # отлавливание ошибки конструкцией tre-except-else
        ...
        ):  # проверка на наличия количества элементов в списке
            raise utils.APIException(
                "Неверно указан формат ввода, формат ввода должен соответствовать условию:"
                "\n <имя валюты, цену которой вы хотите узнать> "
                "<имя валюты, в которой надо узнать цену первой валюты> "
                "<цена валюты>."
            )
        ...
        quote, base, amount = (
            correct_message  # разделение списка на 3 переменные, отвечающие за корректный вывод
        )
        ...
        utils.APIException
    ) as exc:  # если ошибка относится к известным ошибкам, будет срабатывать нужное исключение
        bot.reply_to(message, f"Ошибка пользователя: {exc}")
    except (
        Exception
    ) as exc:  # если ошибка не относится к известной, будет вызываться системная ошибка (исключение)
        bot.reply_to(message, f"Системная ошибка: {exc}")

    else:
        total = utils.Price.get_price(
            quote, base, amount
        )  # вызов статического метода класса Price
...
bot.polling(
    none_stop=True
)  # запуск бота с функцией без остановочного режима, который игнорирует ошибки.
